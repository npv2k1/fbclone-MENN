...
function Comments({ id }) {
  ...
  const [realtimeComments, loading, error] = useCollection(
    db.collection("posts").doc(id).collection("comments").orderBy("timestamp")
  );
  // console.log("id :>> ", id);
  ...
  return (
    <div className="flex flex-col w-full h-[300px] bg-white p-3 space-y-3">
      {/* Comment */}
      <div
        ref={commentsRef}
        className="flex flex-col space-y-3 flex-1 overflow-y-auto scrollbar-hide"
      >
